    ...
    var ccv = {
        ...
        detect_objects: parallable(scriptPath, function (canvas, cascade, interval, min_neighbors) {
            function pre(worker_num) {
                var canvas = this.shared.canvas, interval = this.shared.interval, scale = this.shared.scale,
                    next = this.shared.next, scale_upto = this.shared.scale_upto,
                    pyr = new Array(4 * (scale_upto + 2 * next)), ret = new Array(4 * (scale_upto + 2 * next));
                pyr[0] = canvas, ret[0] = {
                    width: pyr[0].width,
                    height: pyr[0].height,
            ...
            function core(pyr, id, worker_num) {
                var i, j, k, x, y, q, cascade = this.shared.cascade, scale = (this.shared.interval,
                        this.shared.scale), next = this.shared.next, scale_upto = this.shared.scale_upto, scale_x = 1,
                    scale_y = 1, dx = [0, 1, 0, 1], dy = [0, 0, 1, 1], seq = [];
            ...
            this.each(function () {
                function detect() {
                    ...
                    if (time = new Date().getTime(), $$.is("img")) source = new Image(), source.src = $$.attr("src"),
                        source.crossOrigin = $$.attr("crossorigin"), canvas = ccv.pre(source);
                ...
                var $$ = $(this), offset = $$.offset(), position = $$.position(),
